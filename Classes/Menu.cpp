    ...
    while(true) {
        ...
        switch (input) {
            ...
            case 4: {
                ...
                Pedido *pedido = new Pedido(input, turma1, estudante, estudante_troca);
                g->adicionar_pedido(pedido);
                std::cout << "\nPedido adicionado à fila\n\n";
                int v = validar_opcao("Pretende fazer outro pedido para o mesmo estudante?\n[1] Sim\n[2] Não\n");
                if (v == 1){
...
void Menu::fim(){
    ...
    if (!g->get_pedidos().empty()){
        ...
        if (opcao == 1) {
            ...
            if (!g->get_pedidos_falhados().empty()) {
                g->arquivo_pedidos();
            }
        }
    }
...
void Menu::extra(){
    ...
    for (Pedido* p: g->get_pedidos_falhados()){
        ...
        if( g->max_diferenca(g->pesquisa_uc(p->get_turma()->get_codigo_uc())) > Gestao::desequilibrio) {
            pedidos_desequilibro.push_back(p);
            g->remover_pedido_falhado(p);}
    }

    if (pedidos_desequilibro.size() == 0) return;
    ...
    std::cout << '\n';
    int x = validar_opcao("Deseja mudar o desiquilíbrio e tentar novamente?\n[1] Sim\n[2] Não\n");
    if (x == 1){
        int k;
        std::cout << "Limite atual: " << Gestao::desequilibrio << '\n';
        std::cout << "Defina um novo limite: ";
        std::cin >> k;
        while(std::cin.fail() || (k < Gestao::desequilibrio)) {
            if (k == -1) return;
            std::cout << "Input inválido" << '\n';
            std::cout << "Defina um limite válido: ";
            std::cin.clear();
            std::cin.ignore(INT_MAX, '\n');
            std::cin >> k;
        }
        std::cout << '\n';
        Gestao::set_desequilibrio(k);
        for(Pedido* pedido: pedidos_desequilibro){
            g->processar_pedido(pedido);
        }
        if (g->get_pedidos_falhados().empty()) std::cout << "Todos os pedidos foram efetuados com sucesso!\n\n";
        else {
            std::cout << "Pedidos por realizar após ter alterado o limite: " << g->get_pedidos_falhados().size() << "\n\n";
        }
    }
